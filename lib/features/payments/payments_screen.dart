...
class PaymentsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        PgAppBar(title: 'Payments', onBack: () => PgBackScope.pop(context)),
        Expanded(
  ...
  Widget _row(BuildContext context, PgColors c, PaymentRecord r) => GestureDetector(
        ...
        child: Container(
          ...
          child: Row(children: [
            Container(
              ...
              child: Text(r.kind == PaymentKind.service ? '🐾' : '🏡', style: const TextStyle(fontSize: 20)),
            ),
            const SizedBox(width: 13),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(r.title, style: PgText.poppins(15, FontWeight.w700, color: c.text), maxLines: 1, overflow: TextOverflow.ellipsis),
              const SizedBox(height: 2),
              Text(r.subtitle, style: PgText.inter(12.5, FontWeight.w400, color: c.muted), maxLines: 1, overflow: TextOverflow.ellipsis),
              if (PaymentRecord.fmtDateTime(r.paidAtMillis).isNotEmpty) ...[
                const SizedBox(height: 3),
                Text('Paid ${PaymentRecord.fmtDateTime(r.paidAtMillis)}',
                    style: PgText.inter(11.5, FontWeight.w400, color: c.faint)),
              ],
            ])),
            const SizedBox(width: 10),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('₹${r.total}', style: PgText.poppins(15, FontWeight.w800, color: c.text)),
              if (r.isRefunded)
                Text('Refunded', style: PgText.inter(11.5, FontWeight.w700, color: c.heart)),
            ]),
          ]),
        ),
      );
  ...
  Widget _empty(PgColors c) => Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('No payments yet', style: PgText.poppins(18, FontWeight.w700, color: c.text)),
          const SizedBox(height: 4),
          Text('Bookings you pay for will show up here.',
