...
class _MyBookingsScreenState extends ConsumerState<MyBookingsScreen> {
  ...
  Widget build(BuildContext context) {
    final c = context.pg;
    final hasListing =
        ref.watch(currentProProvider).value != null ||
        ref.watch(currentHomestayProvider).value != null;
    final tab = hasListing ? _tab : 0;

    return PgBackScope(
      // Reached via `go` from a push-notification tap and cold-start deep
      // link (stack wiped), but also legitimately pushed from Home/Profile —
      // upToIfEmpty keeps a real pop when a parent is on the stack and only
      // falls back to Home when there is nothing to pop to.
        ...
        body: SafeArea(
          child: Column(
            ...
            children: [
              ...
              Expanded(
                child: tab == 1 ? const ReceivedTab() : const _MyBookingsTab(),
              ),
            ],
          ),
        ),
    ...
    if (services.isEmpty && stays.isEmpty) {
      return Center(
        child: Padding(
          ...
          child: Text(
            'No bookings yet — book a service or a homestay to get started.',
          ...
          for (final b in services)
            _MyBookingRow(
              ...
              onRate: () => context.push(
                Routes.rate,
                extra: ReviewTarget(
                  type: ReviewTargetType.pro,
                  id: b.proId,
                  name: b.proName,
                  subtitle: '${b.serviceType.label} · ${b.dateLabel}',
                  bookingId: b.id,
                ),
              ),
            ),
        ...
        if (stays.isNotEmpty) ...[
          ...
          for (final s in stays)
            _MyBookingRow(
              ...
              name: s.homeName,
              detail:
                  '${s.hostName} · ${HomestayBooking.fmtDay(s.checkIn)} · ${s.nights} nights'
              ...
              showContactHost:
                  stayPhase(s, now) == BookingPhase.upcoming &&
                  !canCancelPaidStay(s, now),
              ...
              onCancel: () => confirmAndRun(
                ...
                confirmLabel: 'Cancel booking',
                action: () => ref
                    .read(homestayBookingRepositoryProvider)
                    .cancelStay(s.id),
              ),
              onRate: () => context.push(
                Routes.rate,
                extra: ReviewTarget(
                  type: ReviewTargetType.homestay,
                  id: s.hostId,
                  name: s.homeName,
                  subtitle: '${s.hostName} · ${s.nights} nights',
                  bookingId: s.id,
                ),
              ),
            ),
...
Future<void> _confirmCancelPaid(
  ...
  final ok = await showDialog<bool>(
    ...
    builder: (dctx) => AlertDialog(
      ...
      title: Text(
        'Cancel this stay?',
        style: PgText.poppins(16, FontWeight.w700, color: c.text),
      ),
      content: Text(
        refund > 0
      ...
      actions: [
        ...
        TextButton(
          ...
          child: Text(
            refund > 0 ? 'Cancel & refund' : 'Cancel anyway',
    ...
    messenger.showSnackBar(
      SnackBar(
        ...
        content: Text(switch (e.message) {
          ...
          _ =>
            "We couldn't confirm this cancellation — check My bookings before trying again.",
        }),
      ),
    );
  }
...
class _MyBookingRow extends StatelessWidget {
  ...
  const _MyBookingRow({
    ...
    this.canPay = false,
    this.onPay,
    this.showContactHost = false,
    this.canCancelPaid = false,
    this.onCancelPaid,
  });
