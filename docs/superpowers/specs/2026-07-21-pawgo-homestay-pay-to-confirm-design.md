...
- **Payment confirms the stay.** `accepted` (host said yes, unpaid) is an *action-needed* state for the guest, phase **`awaitingPayment`** (chip label ships as **"Awaiting payment"** — a neutral state descriptor that reads correctly on the host's Received ledger too; the guest's row pairs it with a **"Pay to confirm"** action button). Only paying (→ status `paid`) makes it a real Upcoming stay. An `accepted` stay whose check-in passes unpaid → **`expired`** (never happened, never rateable). `canRate` stays `completed`-only, and `completed` is now only reachable via `paid` — so rating automatically requires payment.
